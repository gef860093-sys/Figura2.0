...
exports.createAntiSpamSystem = createAntiSpamSystem;
class InProcessBanStore {
    ...
    async recordViolation(clientId) {
        const now = Date.now();
        const ban = this.bans.get(clientId);
        if (ban && now < ban.expiresAt) {
            // Extend ban
            ban.expiresAt = Math.min(ban.expiresAt + this.config.banExtensionMs, ban.createdAt + this.config.maxBanDurationMs);
            ban.violations++;
        }
        else {
            // Create new ban
    ...
    async recordViolation(clientId) {
        ...
        try {
            ...
            if (existing && existing.expiresAt) {
                ...
                if (now < expiresAt) {
                    // Extend ban
                    const newExpiresAt = Math.min(expiresAt + this.config.banExtensionMs, parseInt(existing.createdAt, 10) + this.config.maxBanDurationMs);
            ...
            // Create new ban
            await this.redis.hset(key, 'clientId', clientId, 'expiresAt', now + this.config.banDurationMs, 'violations', 1, 'createdAt', now);
...
function createAntiSpamSystem(redis, config, logger) {
    ...
    const getOrCreateEntry = (clientId, now) => {
    ...
    const middleware = () => {
        return async (req, res, next) => {
            const clientId = req.userInfo?.uuid || req.ip || 'unknown';
            try {
    ...
    const recordViolation = async (clientId) => {
        const now = Date.now();
    ...
    return {
        ...
        liftBan: (clientId) => store.liftBan(clientId),
    };
}
