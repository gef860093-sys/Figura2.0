 ...
 */
function parseStringEnv(value: string | undefined, defaultValue: string, required: boolean = false): string {
  if (value === undefined) {
    if (required) {
 ...
 */
function parseListEnv(value: string | undefined, defaultValue: string[]): string[] {
  if (value === undefined) {
    return defaultValue;
  }
  if (value.trim() === '') {
